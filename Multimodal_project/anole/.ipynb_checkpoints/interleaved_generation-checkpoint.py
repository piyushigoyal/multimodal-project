...
def main(args: argparse.Namespace):
    ...
    for idx, sample in enumerate(samples):
        ...
        image_path = sample["image_path"]  # full or relative path to image
        ...
        for seg_id, (seg_type, seg_tokens) in enumerate(segments):
            if seg_type == "image_seg":
                ...
                img_path = os.path.join(args.save_dir, f"{idx}_{seg_id}.png")
                img.save(img_path)
                print(f"<img: {img_path}>")
            elif seg_type == "text_seg":
                decoded_text = model.decode_text(seg_tokens)[0]
                print(f"Q#{idx} → {decoded_text}")
